...
tpitch_pid.output_limits = (-10, 10)  # Limit pitch correction

TARGET_ALTITUDE = 2.0  # Meters

# Function to send velocity commands to Pixhawk
def send_mavlink_velocity(yaw_rate, pitch_rate):
    msg = pixhawk.mav.set_attitude_target_encode(
        ...
        0  # Thrust (altitude hold handled separately)
    )
    ...
    while True:
        ...
        with MappedArray(request, 'main') as m:
            ...
            for detection in detections:
                ...
                send_mavlink_velocity(yaw_correction, pitch_correction)
                cv2.rectangle(m.array, (x, y), (x + w, y + h), (0, 255, 0), 2)
                cv2.circle(m.array, object_center, 5, (0, 0, 255), -1)
                cv2.putText(m.array, f"Yaw: {yaw_correction:.2f}, Pitch: {pitch_correction:.2f}", (x, y - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 1)
